...
Multiple outputs are also supported. You can use `produces()` both inline (when the tool accepts absolute paths) and out-of-line (for outputs that aren't specified on the command line):

```javascript
to("compile", ({T}) => {
    produces("output.d");  // Out-of-line: dependency file generated by -MMD
    return T`gcc -c -MMD ${depends("source.c")} -o ${produces("output.o")}`;  // Inline: object file
});
```

This example demonstrates:
- **Out-of-line `produces()`**: For `output.d`, which is generated automatically by gcc's `-MMD` flag
- **Inline `produces()`**: For `output.o`, which is specified with the `-o` flag in the command
