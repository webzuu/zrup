...
var _CommandRecipe_commandBuilder;
...
export class CommandRecipe extends Recipe {
    constructor(commandBuilder) {
        super();
        _CommandRecipe_commandBuilder.set(this, void 0);
        this.stdoutChunks = [];
        this.stderrChunks = [];
        this.combinedChunks = [];
        __classPrivateFieldSet(this, _CommandRecipe_commandBuilder, commandBuilder.bind(null), "f");
    }
    createChildProcess(job, config) {
        let { exec, shell, args, cwd, out, err, combined } = config;
        const options = {};
        //resolve cwd
        const resolved = resolveArtifacts(job.build.artifactManager, job.rule.module, false, ...[cwd ?? (job.rule.module.name + '+')].flat(flattenDepth))[0];
    ...
    async concretizeSpecFor(job) {
        ...
        const builderParams = {
            exec: (cmdString, ...argItems) => {
                spec.exec = cmdString;
                spec.args.push(...resolveExceptStrings(...argItems.flat(flattenDepth)).map(_ => '' + _));
            },
            shell: (...argItems) => {
                spec.exec = resolveExceptStrings(...argItems.flat(flattenDepth)).map(_ => '' + _).join(" ");
                spec.shell = true;
            },
            args: (...argItems) => {
                spec.args.push(...resolveExceptStrings(...argItems.flat(flattenDepth)).map(_ => '' + _));
        ...
        __classPrivateFieldGet(this, _CommandRecipe_commandBuilder, "f").call(this, builderParams);
        return spec;
    }
    ...
    static fromSimpleDescriptor(module, descriptor) {
        return new CommandRecipe((C) => {
            ...
            const commandSegments = [descriptor.cmd].flat(8);
            ...
            if ('args' in descriptor) {
                C.args(...[descriptor.args].flat(8));
            }
            if ('cwd' in descriptor) {
                const cwd = [descriptor.cwd].flat(flattenDepth);
                if (cwd.length !== 1) {
            ...
            if ('out' in descriptor)
                [descriptor.out].flat(8).forEach(C.out.bind(C));
            if ('err' in descriptor)
                [descriptor.err].flat(8).forEach(C.err.bind(C));
            if ('combined' in descriptor)
                [descriptor.combined].flat(8).forEach(C.combined.bind(C));
        });
    }
...
_CommandRecipe_commandBuilder = new WeakMap();
/***/
