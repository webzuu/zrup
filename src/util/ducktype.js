...
export default function recursive(callback)
{
    ...
    const numIndirects = callback.length;

    const indirects = [];
    for(let i=numIndirects; i; --i) {
    ...
    if (matrix.length !== numIndirects) throw new Error(
        `Recursive ducktype was declared with ${numIndirects} indirection(s), but the callback returned ${matrix.length} item(s)`
    );
    for(let i=0; i<numIndirects; ++i) {
